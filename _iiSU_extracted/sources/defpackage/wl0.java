...
public final class wl0 {
    ...
    public wl0(float f, float f2, float f3, float f4, float f5, float f6) {
        this.a = f;
        this.b = f2;
        this.c = f3;
        this.d = f4;
    ...
    public static wl0 a(int i) {
        tx8 tx8Var = tx8.k;
        float fQ = zz1.Q(Color.red(i));
        float fQ2 = zz1.Q(Color.green(i));
        float fQ3 = zz1.Q(Color.blue(i));
        float[][] fArr = zz1.j;
        float[] fArr2 = fArr[0];
        ...
        float fPow4 = ((float) Math.pow((f18 * tx8Var.b) / f9, tx8Var.j * f8)) * 100.0f;
        Math.sqrt(fPow4 / 100.0f);
        float f20 = f9 + 4.0f;
        float fPow5 = ((float) Math.pow(1.64d - Math.pow(0.29d, tx8Var.f), 0.73d)) * ((float) Math.pow((((((((float) (Math.cos(((((double) (((double) fAtan2) < 20.14d ? 360.0f + fAtan2 : fAtan2)) * 3.141592653589793d) / 180.0d) + 2.0d) + 3.8d)) * 0.25f) * 3846.1538f) * tx8Var.e) * tx8Var.c) * ((float) Math.sqrt((f15 * f15) + (f14 * f14)))) / (f17 + 0.305f), 0.9d)) * ((float) Math.sqrt(((double) fPow4) / 100.0d));
        Math.sqrt((r0 * f8) / f20);
        float f21 = (1.7f * fPow4) / ((0.007f * fPow4) + 1.0f);
        float fLog = ((float) Math.log((f7 * fPow5 * 0.0228f) + 1.0f)) * 43.85965f;
        double d2 = f19;
        return new wl0(fAtan2, fPow5, fPow4, f21, fLog * ((float) Math.cos(d2)), fLog * ((float) Math.sin(d2)));
    }

    public static wl0 b(float f, float f2, float f3) {
        tx8 tx8Var = tx8.k;
        float f4 = tx8Var.d;
        Math.sqrt(((double) f) / 100.0d);
        float f5 = tx8Var.a + 4.0f;
        float f6 = tx8Var.i * f2;
        Math.sqrt(((f2 / ((float) Math.sqrt(r1))) * tx8Var.d) / f5);
        float f7 = (1.7f * f) / ((0.007f * f) + 1.0f);
        float fLog = ((float) Math.log((((double) f6) * 0.0228d) + 1.0d)) * 43.85965f;
        double d = (3.1415927f * f3) / 180.0f;
        return new wl0(f3, f2, f, f7, fLog * ((float) Math.cos(d)), fLog * ((float) Math.sin(d)));
    }
    ...
    */
    public final int c(defpackage.tx8 r20) {
        /*
            Method dump skipped, instruction units count: 377
            To view this dump change 'Code comments level' option to 'DEBUG'
        */
        throw new UnsupportedOperationException("Method not decompiled: defpackage.wl0.c(tx8):int");
    }
}
