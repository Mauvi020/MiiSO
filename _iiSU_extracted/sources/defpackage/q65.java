...
public final class q65 {
    ...
    public static final void c(float[] fArr, eh5 eh5Var) {
        if (fArr.length < 16) {
            return;
        }
        float f = fArr[0];
        ...
        eh5Var.a = Math.min(f19, Math.min(f26, Math.min(f32, f37)));
        eh5Var.b = Math.min(f22, Math.min(f28, Math.min(f34, f38)));
        eh5Var.c = Math.max(f19, Math.max(f26, Math.max(f32, f37)));
        eh5Var.d = Math.max(f22, Math.max(f28, Math.max(f34, f38)));
    }
